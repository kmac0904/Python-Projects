...
class FinancialPlannerApp(QtWidgets.QMainWindow):
    ...
    def calculate_how_long(self):
        try:
            ...
            current_amount = 0
            years = 0
            for year in range(1, 101):  # Iterate up to a maximum of 100 years
                current_amount += annual_amount
                current_amount += current_amount * rate_of_return
                if current_amount >= target_amount:
                    years = year
                    break  # Exit the loop early if target is reached

            # Display result
            if current_amount >= target_amount:
                self.label_result.setText(f"Years to Target amount: {years}")
            else:
                self.label_result.setText("Target amount not achievable within 100 years.")
